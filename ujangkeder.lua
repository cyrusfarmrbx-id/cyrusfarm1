...
-- [PERMANENT HIDE PLAYER & TOTEM EFFECTS] Mulai
local chars = workspace:FindFirstChild("Characters")
local totems = workspace:FindFirstChild("Totems")

-- 1. FUNGSI PEMBERSIH EFEK TOTEM (Berdasarkan Gambar Explorer)
local function bersihkanEfekTotem(obj)
    if obj.Name == "Ring" or obj.Name == "Gems" then
    ...
    elseif string.find(obj.Name, "Cube") or string.find(obj.Name, "Cylinder") or string.find(obj.Name, "Torus") or obj.Name == "neon" then
...
-- AKTIFKAN PEMANTAUAN TOTEM SEPANJANG WAKTU
if totems then
    for _, obj in ipairs(totems:GetDescendants()) do bersihkanEfekTotem(obj) end
    totems.DescendantAdded:Connect(bersihkanEfekTotem) -- Memantau otomatis setiap Anda spawn totem baru
end

-- 2. PEMANTAUAN OTOMATIS REAL-TIME UNTUK PEMAIN BARU (Tanpa Loop)
local function sembunyikanKarakter(c)
    if c:IsA("Model") and c ~= LocalPlayer.Character then
        -- Beri jeda singkat agar semua aset tubuh/baju pemain selesai dimuat game
        task.wait(0.2) 
        ...
        -- Jaga-jaga jika pemain tersebut mengganti kostum/aksesoris di tengah game
...
if chars then
    -- Sembunyikan semua pemain yang sudah ada di dalam server saat ini
    for _, c in ipairs(chars:GetChildren()) do 
        sembunyikanKarakter(c) 
    end
    -- Otomatis langsung tangkap dan sembunyikan setiap ada pemain baru lahir (Spawn)
    chars.ChildAdded:Connect(sembunyikanKarakter) 
end
-- [PERMANENT HIDE PLAYER & TOTEM EFFECTS] Selesai
